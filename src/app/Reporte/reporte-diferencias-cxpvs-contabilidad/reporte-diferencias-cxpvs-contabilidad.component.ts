...
export class ReporteDiferenciasCXPvsContabilidadComponent {
  ...
  V_Imprimir(): void {
    ...
    document.getElementById("btnReporte-DiferenciasCXPvsCont")?.setAttribute("disabled", "disabled");
    ...
    this.GET.GetDiferenciasCXPvsCONT(this.cFunciones.DateFormat(this.val.Get("txtFecha1").value, "yyyy-MM-dd"), this.val.Get("cmbMoneda").value).subscribe(
      {
        next: (data) => {
        ...
        error: (err) => {
          ...
          dialogRef.close();
          document.getElementById("btnReporte-DiferenciasCXPvsCont")?.removeAttribute("disabled");
  ...
  ngDoCheck(): void {
    ...
    this.val.addFocus("cmbMoneda", "btnReporte-DiferenciasCXPvsCont", "click");
