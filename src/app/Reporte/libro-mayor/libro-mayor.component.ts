...
export class LibroMayorComponent {
  ...
  V_Imprimir(): void {
    ...
    document.getElementById("btnReporte-Libro-Mayor")?.setAttribute("disabled", "disabled");
    ...
    this.GET.GetLibroMayor(this.cFunciones.DateFormat(this.val.Get("txtFecha").value, "yyyy-MM-dd"),this.val.Get("cmbOpcion").value,this.val.Get("cmbMoneda").value[0]).subscribe(      
      {
        next: (data) => {
        ...
        error: (err) => {
          ...
          dialogRef.close();
          document.getElementById("btnReporte-Libro-Mayor")?.removeAttribute("disabled");
  ...
  ngDoCheck(): void {
    ...
    this.val.addFocus("cmbOpcion", "btnReporte-Libro-Mayor", "click");   
