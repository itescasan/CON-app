...
export class DiferenciasComponent {
  ...
  V_Imprimir(): void {
    ...
    document.getElementById("btnReporte-Diferencias")?.setAttribute("disabled", "disabled");
    ...
    this.GET.GetDiferencias(this.cFunciones.DateFormat(this.val.Get("txtFecha1").value, "yyyy-MM-dd"), this.val.Get("cmbMoneda").value).subscribe(
      {
        next: (data) => {
        ...
        error: (err) => {
          ...
          dialogRef.close();
          document.getElementById("btnReporte-Diferencias")?.removeAttribute("disabled");
  ...
  ngDoCheck(): void {
    ...
    this.val.addFocus("cmbMoneda", "btnReporte-Diferencias", "click");
