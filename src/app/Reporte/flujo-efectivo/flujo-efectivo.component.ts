...
export class FlujoEfectivoComponent {
  ...
  V_Imprimir(): void {
    ...
    this.GET.GetFlujoEfectivo(this.cFunciones.DateFormat(this.val.Get("txtFecha").value, "yyyy-MM-dd"),this.cFunciones.DateFormat(this.val.Get("txtFechaF").value, "yyyy-MM-dd"),true,this.val.Get("cmbMoneda").value[0]).subscribe(      
      {
        next: (data) => {
  ...
  ngDoCheck(): void {
    ...
    this.val.addFocus("txtFechaF", "cmbMoneda", undefined);
